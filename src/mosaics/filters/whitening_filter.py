...
def calculate_radial_sum(
    ...
    NOTE: For power spectral density, need to abs or ^2 image before passing
    ...
    if interpolation == "nearest":
        ...
        values_sum = np.bincount(
            indexes[mask], weights=image[mask], minlength=num_bins
        )
        counts_sum = np.bincount(indexes[mask], minlength=num_bins)

    elif interpolation == "linear":
        # TODO: Possibly move the common bincount routine to a separate func
        # for reduction of code duplication
...
def compute_power_spectral_density_1D(
    ...
    Args:
        ...
        number of bins is automatically calculated based on the image
        dimensions.
        in_fourier_space (bool): If True, the image is assumed to already be in
            Fourier space. Default is False.
